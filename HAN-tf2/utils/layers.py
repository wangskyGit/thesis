...
def attn_head(seq, out_sz, bias_mat, activation, in_drop=0.0, coef_drop=0.0, residual=False,
    ...
    with tf.compat.v1.name_scope('my_attn'):
        ...
        bias = tf.zeros_initializer()(shape=[vals.get_shape()[-1],],dtype=vals.dtype)
        ret = tf.nn.bias_add(vals,bias)

        # residual connection
        if residual:
            if seq.shape[-1] != ret.shape[-1]:
                ret = ret + tf.compat.v1.layers.onv1d(seq, ret.shape[-1], 1)  # activation
            else:
                seq_fts = ret + seq
        if return_coef:
...
def sp_attn_head(seq, out_sz, adj_mat, activation, nb_nodes, in_drop=0.0, coef_drop=0.0, residual=False):
    with tf.compat.v1.name_scope('sp_attn'):
        ...
        f_1 = tf.compat.v1.layers.conv1d(seq_fts, 1, 1)
        f_2 = tf.compat.v1.layers.conv1d(seq_fts, 1, 1)
        logits = tf.sparse.add(a=adj_mat * f_1, b=adj_mat *
                               tf.transpose(a=f_2, perm=[0, 2, 1]))
